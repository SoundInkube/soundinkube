  ...
  (
    ...
    return (
      ...
      >
        ...
        <div className="text-white grid gap-1.5">
                ...
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    <div className={cn('flex flex-1 justify-between leading-none', nestLabel ? 'items-end' : 'items-center')}>
                      <div className="text-white grid gap-1.5">
                        {nestLabel ? tooltipLabel : null}
                        <span className="text-white text-muted-foreground">{itemConfig?.label || item.name}</span>
                      </div>
                      {item.value && (
                        <span className="text-white font-mono font-medium tabular-nums text-foreground">{item.value.toLocaleString()}</span>
                      )}
                    </div>
                  </>
                )}
        ...
        return (
          <div key={item.value} className={cn('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground')}>
            {itemConfig?.icon && !hideIcon ? (
              ...
              <div
                className="text-white h-2 w-2 shrink-0 rounded-[2px]"
                style={{
